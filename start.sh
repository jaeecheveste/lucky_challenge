...
if [ "$CLEAN_CONTAINERS" == "true" ]; then
  ...
  docker network remove lucky_net
fi

echo "Creating network"
docker network create lucky_net

echo "SETTING UP DATABASE"
docker run --net lucky_net --name $SERVER -e POSTGRES_PASSWORD=$PW \
  -e PGPASSWORD=$PW \
  -p 5432:5432 \
  -d postgres
...
docker run --net lucky_net --name $REDIS_SERVER \
  -e PGPASSWORD=$PW \
  -p 6379:6379 \
  -d redis
...
if [ "$RUN_MIGRATIONS" == "true" ]; then
  ...
  docker run --net lucky_net --name $MIGRATIONS_SERVER -d $MIGRATIONS_SERVER
fi

echo "SETTING UP API"
docker build -t $API_SERVER .
docker run --net lucky_net --name $API_SERVER -p 3000:3000 -d $API_SERVER
